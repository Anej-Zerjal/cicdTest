...
class KronotermCloudApi:
    ...
    def __init__(self, username: str, password: str, hass: HomeAssistant):
        ...
        self.modbus_client = pymodbus.client.ModbusSerialClient("/dev/ttyUSB0",  baudrate=115200)
    ...
    async def get_alarms_data_only(self, alarms_data: dict | None = None) -> dict:
    ...
    async def get_outside_temperature(self) -> float:
        ...
        #log.debug(f"Outside temperature: {data}")
    ...
    async def get_outlet_temp(self) -> float:
        ...
        """
        rewiev_data = await self.get_system_review_data()
        dv_temp = rewiev_data["CurrentFunctionData"][0]["dv_temp"]
    ...
    async def get_outside_temperature_mqtt(self):
        self.modbus_client.close()
        self.modbus_client.connect()

        outside_temp_addr = 2103
        ...
        log.debug("Zunanja temperatura:")
        for strTmp in [u'{:.1f}\N{DEGREE SIGN}C'.format((t - (t >> 15 << 16)) / 10) for t in rr.registers]:
            log.debug(strTmp)

        tmp = rr.registers[0]
        self.modbus_client.close()
        return (tmp - (tmp >> 15 << 16)) / 10

    async def set_loop_1_temp(self):
        self.modbus_client.close()
        self.modbus_client.connect()

        loop1_temp_addr = 2309
        value_to_write = 253
        response = await asyncio.to_thread(
            self.modbus_client.write_register,
            address=loop1_temp_addr - 1,
            value=value_to_write,
            slave=MODBUS_SLAVE_ID
        )

        self.modbus_client.close()
        return "Nastavljeno na 25.3 stopinj"
    ...
    map_label_to_function = {
        ...
        "get_outside_temperature_mqtt": get_outside_temperature_mqtt,
        "set_loop_1_temp": set_loop_1_temp,
    }


    async def invoke_kronoterm_action(self, action: KronotermAction):
        """Invokes an action on the Kronoterm heat pump."""
        handler = self.map_label_to_function.get(action.action_label)
        if handler is None:
            raise ValueError(f"Action '{action.action_label}' not supported")
